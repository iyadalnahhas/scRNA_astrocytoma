...
Wang_tumor_seurat$THY1_Wang <- ifelse(THY1_Wang > 0, "THY1+", "THY1-")
EGFR_Wang <- FetchData(Wang_tumor_seurat, vars = "EGFR")
Wang_tumor_seurat$EGFR_Wang<- ifelse(EGFR_Wang > 0, "EGFR+", "EGFR-")
CD24_Wang <- FetchData(Wang_tumor_seurat, vars = "CD24")
Wang_tumor_seurat$CD24_Wang <- ifelse(CD24_Wang > 0, "CD24+", "CD24-")
#MGMT_Wang <- FetchData(Wang_tumor_seurat, vars = "MGMT")
#Wang_tumor_seurat$MGMT_Wang <- ifelse(MGMT_Wang > 0, "MGMT+", "MGMT-")
...
Wang_tumor_seurat$TERT_Wang <- ifelse(TERT_Wang > 0, "TERT+", "TERT-")
...
write.table(GSEA.df.EGFRpos.p,"GSEA.df.EGFRpos.p.tsv", sep = "\t", col.names= TRUE, row.names = FALSE, quote = FALSE)
...
sig_EGFRpos_combined <- sig_EGFRpos_primary[sig_EGFRpos_primary$Gene %in% sig_EGFRpos_recurrent$Gene, ]

EnhancedVolcano(sig_EGFRpos_combined,
                x = "avg_log2FC",
                y = "p_val_adj",
                title = "Differentially expressed markers between EGFR+ and EGFR- cells",
                subtitle = "EGFR amplified samples",
                pCutoff = 10e-12,
                pCutoffCol = "p_val_adj",
                FCcutoff = 0.5,
                pointSize = 2.0,
                labSize = 3.0,
                drawConnectors = TRUE,
                lab = rownames(sig_EGFRpos_combined))

ggsave("EGFRpos_combined.tiff", width = 12, height = 6)
...
write.table(GSEA.df.EGFRpos.r,"GSEA.df.EGFRpos.r.tsv", sep = "\t", col.names= TRUE, row.names = FALSE, quote = FALSE)
...
write.table(GSEA.df.EGFRneg.p,"GSEA.df.EGFRneg.p.tsv", sep = "\t", col.names= TRUE, row.names = FALSE, quote = FALSE)
...
sig_EGFRneg_combined <- sig_EGFRneg_primary[sig_EGFRneg_primary$Gene %in% sig_EGFRneg_recurrent$Gene , ]

EnhancedVolcano(sig_EGFRneg_combined,
                x = "avg_log2FC",
                y = "p_val_adj",
                title = "Differentially expressed markers between EGFR+ and EGFR- cells",
                subtitle = "EGFR wild-type samples",
                pCutoff = 10e-12,
                pCutoffCol = "p_val_adj",
                FCcutoff = 0.5,
                pointSize = 2.0,
                labSize = 3.0,
                lab = rownames(sig_EGFRneg_combined))
ggsave("EGFRneg_combined.tiff", width = 12, height = 6)
...
write.table(GSEA.df.EGFRneg.r,"GSEA.df.EGFRneg.r.tsv", sep = "\t", col.names= TRUE, row.names = FALSE, quote = FALSE)
...
Venteicher_tumor_seurat$THY1_Venteicher <- ifelse(THY1_Venteicher > 0, "THY1+", "THY1-")
EGFR_Venteicher <- FetchData(Venteicher_tumor_seurat, vars = "EGFR")
Venteicher_tumor_seurat$EGFR_Venteicher<- ifelse(EGFR_Venteicher > 0, "EGFR+", "EGFR-")
CD24_Venteicher <- FetchData(Venteicher_tumor_seurat, vars = "CD24")
Venteicher_tumor_seurat$CD24_Venteicher <- ifelse(CD24_Venteicher > 0, "CD24+", "CD24-")
#MGMT_Venteicher <- FetchData(Venteicher_tumor_seurat, vars = "MGMT")
#Venteicher_tumor_seurat$MGMT_Venteicher <- ifelse(MGMT_Venteicher > 0, "MGMT+", "MGMT-")
IDH1_Venteicher <- FetchData(Venteicher_tumor_seurat, vars = "IDH1")
Venteicher_tumor_seurat$IDH1_Venteicher <- ifelse(IDH1_Venteicher > 0, "IDH1+", "IDH1-")
PDGFRA_Venteicher <- FetchData(Venteicher_tumor_seurat, vars = "PDGFRA")
Venteicher_tumor_seurat$PDGFRA_Venteicher <- ifelse(PDGFRA_Venteicher > 0, "PDGFRA+", "PDGFRA-")
...
We then aimed to apple the CD24/THY1/EGFR classification to IDH mutant/1p19q co-deleted samples from the Tirosh study. Similar to IDH mutant astrocytoma, of 1p19q co-deleted oligodendrogliomas, 80.19% of cells were THY1+, 15% of cells were THY1-CD24+ and 4.72% of cells were THY1-CD24-. Figure 1 shows the cellular composition of all primary IDH mutant astrocytoma, 1p19q co-deleted oligodendroglioma and IDHwt glioblastoma samples.
...
Tirosh_tumor_seurat$THY1_Tirosh <- ifelse(THY1_Tirosh > 0, "THY1+", "THY1-")
EGFR_Tirosh <- FetchData(Tirosh_tumor_seurat, vars = "EGFR")
Tirosh_tumor_seurat$EGFR_Tirosh<- ifelse(EGFR_Tirosh > 0, "EGFR+", "EGFR-")
CD24_Tirosh <- FetchData(Tirosh_tumor_seurat, vars = "CD24")
Tirosh_tumor_seurat$CD24_Tirosh <- ifelse(CD24_Tirosh > 0, "CD24+", "CD24-")
MGMT_Tirosh <- FetchData(Tirosh_tumor_seurat, vars = "MGMT")
Tirosh_tumor_seurat$MGMT_Tirosh <- ifelse(MGMT_Tirosh > 0, "MGMT+", "MGMT-")
IDH1_Tirosh <- FetchData(Tirosh_tumor_seurat, vars = "IDH1")
Tirosh_tumor_seurat$IDH1_Tirosh <- ifelse(IDH1_Tirosh > 0, "IDH1+", "IDH1-")
PDGFRA_Tirosh <- FetchData(Tirosh_tumor_seurat, vars = "PDGFRA")
Tirosh_tumor_seurat$PDGFRA_Tirosh <- ifelse(PDGFRA_Tirosh > 0, "PDGFRA+", "PDGFRA-")
...
all_cell_types <- unique(Tirosh_tumor_seurat$cell)

# Define a function to compute percentages, ensuring all cell types are included
compute_percentage <- function(sample_name) {
  counts <- table(factor(Tirosh_tumor_seurat$cell[Tirosh_tumor_seurat$sample == sample_name], levels = all_cell_types))
  percentages <- round(counts / sum(counts) * 100, 2)  # Convert to percentage
  return(percentages)
}

# Compute percentages for each sample
Result1 <- compute_percentage("MGH36")
Result2 <- compute_percentage("MGH53")
Result3 <- compute_percentage("MGH54")
Result4 <- compute_percentage("MGH60")
Result5 <- compute_percentage("MGH93")
Result6 <- compute_percentage("MGH97")  # This will now include 0s instead of missing

# Combine results into a data frame
CellGroupPerSample_Tirosh <- as.data.frame(cbind(Result1, Result2, Result3, Result4, Result5, Result6))
...
CellGroupIDHwt <- CellGroupIDHwt %>%
  mutate(cellular_group = case_when(
    row_number() %in% c(3, 4) ~ "CD24+THY1-",  # Merging rows 3 & 4
    row_number() %in% c(5, 6) ~ "THY1+",       # Merging rows 5 & 6
    TRUE ~ as.character(rownames(.))           # Keep other rows unchanged
  )) %>%
  group_by(cellular_group) %>% 
  summarise(across(where(is.numeric), sum, na.rm = TRUE)) 

colnames(CellGroupIDHwt) <- c("cellular_group","IDHwt_EGFRamp", "IDHwt_EGFRwt")
...
CellGroupIDHmutant <- CellGroupIDHmutant %>%
  mutate(cellular_group = case_when(
    row_number() %in% c(3, 4) ~ "CD24+THY1-",  # Merging rows 3 & 4
    row_number() %in% c(5, 6) ~ "THY1+",       # Merging rows 5 & 6
    TRUE ~ as.character(rownames(.))           # Keep other rows unchanged
...
colnames(CellGroupIDHmutant) <- c("cellular_group","IDHII", "IDHIII_primary", "IDHIV_primary")
CellGroupIDHmutant$IDHmutant <- rowMeans(CellGroupIDHmutant[,2:4])
CellGroupIDHmutant <- CellGroupIDHmutant %>% select(1, 5)

CellGroupPerSample_Tirosh <- as.data.frame(cbind(Result1, Result2, Result3, Result4, Result5, Result6))
colnames(CellGroupPerSample_Tirosh) <- c("MGH36", "MGH53", "MGH54", "MGH60", "MGH93","MGH94")

CellGroupOligo <- CellGroupPerSample_Tirosh %>%
  mutate(cellular_group = case_when(
    row_number() %in% c(2, 5) ~ "CD24+THY1-",  # Merging rows 3 & 4
    row_number() %in% c(1, 3) ~ "THY1+",       # Merging rows 5 & 6
    TRUE ~ as.character(rownames(.))           # Keep other rows unchanged
  )) %>%
  group_by(cellular_group) %>% 
  summarise(across(where(is.numeric), sum, na.rm = TRUE)) 

CellGroupOligo$Oligo <- rowMeans(CellGroupOligo[,2:7])
CellGroupOligo <- CellGroupOligo %>% select(1, 8)
...
# Save plot
ggsave("cellular_makeup_all.tiff", width = 10, height = 8, dpi = 300)
